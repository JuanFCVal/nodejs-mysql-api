...
router.get("/bandera/", (req, res) => {
  mysqlConnection.query("SELECT * FROM bandera", (err, rows, fields) => {//Sentencia y lo que podemos obtener
    if (!err) {
      res.json(rows);
    } else {
...
router.get("/banderas/:id/donaciones", (req, res) => {
  ...
  mysqlConnection.query("SELECT Tipo,idBandera FROM bandera b INNER JOIN bandera_has_tipodonacion bd ON b.idBandera =  bd.bandera_idBandera INNER JOIN  tipodonacion td ON bd.tipodonacion_idTipoDonacion = td.idTipoDonacion WHERE idBandera = ?",[id], (err, rows, fields) => {//Sentencia y lo que podemos obtener
    if (!err) {
      res.json(rows);
    } else {
      console.log("No se pudo obtener las banderas " + err);
...
router.get("/banderas/:id/evidencias", (req, res) => {
  const { id } = req.params;
  const { id1 } = req.params;
  mysqlConnection.query("SELECT Descripcion, Imagen1, Imagen2, Imagen3, idBandera FROM bandera b , evidencias e WHERE e.idEvidencias = b.Evidencias_idEvidencias and idBandera=? ",[id], (err, rows, fields) => {//Sentencia y lo que podemos obtener
    if (!err) {
      res.json(rows);
    } else {
      console.log("No se pudo obtener las banderas " + err);
...
router.get("/bandera/:id", (req, res) => {
  ...
  mysqlConnection.query('SELECT * FROM bandera WHERE idBandera = ?', [id], (err, rows, fields) => { ////Sentencia y que puede devolver
    if (!err) {
      res.json(rows[0]);
    } else {
      console.log("No se pudo obtener las banderas " + err);
