...
router.get("/donacion/valido", (req, res) => {
  mysqlConnection.query("SELECT * FROM donacion where estado=1", (err, rows, fields) => {//Sentencia y lo que podemos obtener
    if (!err) {
      res.json(rows);
    } else {
      console.log("No se pudo obtener la lista donaciones " + err);
...
router.post('/donacion/', (req, res)=>{
    const {idDonacion, estado, fecha, Usuario_cedula,tipo1,tipo2,tipo3 } = req.body //Obtenemos el usuario desde la aplicacion 
    const query = ` 

    CALL AgregaroEditarDonacion(?,?,?,?,?,?,?);
    `;
    mysqlConnection.query(query, [idDonacion, estado, fecha, Usuario_cedula,tipo1,tipo2,tipo3], (err, rows, fields)=>{
            if (!err) {
...
router.delete('/donacion/:id',(req, res) => {
    ...
    mysqlConnection.query('DELETE FROM donacion_has_tipodonacion WHERE donacion_idDonacion = ?', [id], (err, rows, fields) => {
      if (!err) {
